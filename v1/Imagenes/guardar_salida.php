...
if (isset($_FILES["factura"]) && $_FILES["factura"]["error"] == 0) {
    $carpeta = "uploads/";
    $permitidos = ['image/jpeg', 'image/png']; // Solo imágenes
...
$stmt = $conexion->prepare("INSERT INTO salidas (Tipo_salida, Monto, Fecha, Factura, id_usuario) VALUES (?, ?, ?, ?, ?)");
$stmt->bind_param("sdssi", $tipo, $monto, $fecha, $ruta, $id_usuario);

if ($stmt->execute()) {
    echo json_encode(["success" => true, "id_salida" => $conexion->insert_id]);
} else {
